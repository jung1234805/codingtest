...
start_x, start_y, start_see=map(int,input().split())
...
while True:
    #시작 위치 확인
    nx=start_x+dx[start_see]
    ny=start_y+dy[start_see]
    #앞이 바다이거나 이미 간 곳일 때
    if array[nx][ny]!=0:
        start_see=start_see+1
    ...
    if turn_time==4:
        if array[start_x-dx[start_see]][start_y-dy[start_see]]==2:
            start_x=start_x-dx[start_see]
            start_y=start_y-dy[start_see]
        elif array[start_x-dx[start_see]][start_y-dy[start_see]]==1:
            break
